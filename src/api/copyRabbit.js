...
function copyRabbit(target, toAccount, pwd) {
	var user = new RabbitUser(toAccount, pwd);
	const url = 'http://www.rabbitpre.com/template/preview/' + target.split('#')[1] + '?mobile=1';
	...
	function loginSuccess(res) {
		...
		return user.getSession().then(res=>rabbitSevice.getUserInfo({'x-jwt-token': user.info.token}))
					...
					.then(res=>rabbitSevice.createTemplate(data))
					.then(res=> {
